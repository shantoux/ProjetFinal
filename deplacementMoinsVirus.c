...
//Version qui n'utilise pas de tableau pour virus//
  ...
  Case emplacement[nrow][ncol])
{
    ...
    for (i = 0; i < *cpt_lambda; i++)
    {
        ...
        {
            emplacement[lambda[i].localisation.y][lambda[i].localisation.x].PV_virus = 0;
            infection(&(lambda[i]));
...
////////////////////// Lecture du tableau soignant ////////////////////////////////////////////////
    ...
    int j=0;
    for (i = 0; i < N; i++) //lecture matrice et décrémentation vie virus
    {
      for (j = 0; j < M; j++)
      {
